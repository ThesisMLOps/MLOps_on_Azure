...
def parse_args():
    ...
    parser.add_argument("--train_data_rai", type=str, help="Path to mltable train dataset")
    parser.add_argument("--test_data_rai", type=str, help="Path to mltable test dataset")    
...
# Rewrite_for_RAI
# total_units Object
# 'co-applicant_ethnicity-5', 'co-applicant_ethnicity-4', 'co-applicant_race-5'
# df.dropna(subset=['co-applicant_race-5'])
# Code 8888—Not applicable
# Code 9999—No co-applicant
# no way to determine race: 8035

def delete_columns(df):
    df = df.drop(columns=['activity_year', 'lei', 'derived_msa-md', 'state_code', 'county_code', 'census_tract', 'tract_population',
                          'tract_minority_population_percent', 'ffiec_msa_md_median_family_income', 'tract_to_msa_income_percentage',
                          'tract_owner_occupied_units', 'tract_one_to_four_family_homes', 'tract_median_age_of_housing_units', 'applicant_credit_score_type',
                          'co-applicant_credit_score_type', 'applicant_ethnicity_observed', 'co-applicant_ethnicity_observed', 'applicant_race_observed',
                          'co-applicant_race_observed', 'applicant_sex_observed', 'co-applicant_sex_observed', 'submission_of_application',
                          'aus-1', 'aus-2', 'aus-3', 'aus-4', 'aus-5', 'initially_payable_to_institution', 'applicant_race-3', 'applicant_race-4',
                          'applicant_race-5', 'co-applicant_race-3', 'co-applicant_race-4','co-applicant_race-5', 'co-applicant_ethnicity-2',
                          'co-applicant_ethnicity-3', 'co-applicant_ethnicity-4', 'co-applicant_ethnicity-5', 'applicant_ethnicity-2',
                          'applicant_ethnicity-3', 'applicant_ethnicity-4', 'applicant_ethnicity-5', 'denial_reason-1', 'denial_reason-2',
                          'denial_reason-3', 'denial_reason-4', 'multifamily_affordable_units', 'manufactured_home_land_property_interest',
                          'manufactured_home_secured_property_type', 'purchaser_type', 'derived_loan_product_type', 'derived_dwelling_category',
                          'interest_rate', 'rate_spread', 'hoepa_status', 'origination_charges', 'total_points_and_fees', 'total_loan_costs',
                          'discount_points', 'lender_credits', 'prepayment_penalty_term', 'intro_rate_period', 'construction_method',
                          'other_nonamortizing_features', 'negative_amortization', 'conforming_loan_limit'
                         ])
    return df
...
def create_derived_age_above_62(df):
    ...
    choices = [1, 0, 10, 1, 0, 8888]
    df['derived_age_above_62'] = np.select(conditions, choices, default=1000)
    return df

#...df['co-applicant_age'] == '9999')), 'Yes', 'No')
def create_derived_age_below_25(df):
    ...
    df['derived_age_below_25'] = np.where((df['applicant_age'] == '<25') & ((df['co-applicant_age'] == '<25') | (df['co-applicant_age'] == '9999')), 1, 0)
    return df
...
def create_derived_race_revisited(df):
    ...
    choices = [5, 1, 3, 2, 1, 4, 8888, 33, 55]
    df['derived_race_revisited'] = np.select(conditions, choices, default = 30)
    return df
...
def turn_action_taken_into_binary_target_variable(df):
    ...
    # ------------------------------------------------------------------------------------------------
    df.loc[(df['action_taken'].isin([1, 2, 8])), 'action_taken'] = 1 #'Approved'
    df.loc[(df['action_taken'].isin([3, 7])), 'action_taken'] = 0 #'Denied'                              
    return df
    
# According to 2023 "Guide To HMDA Reporting. Getting It Right!" applications that fall under the categories filtered out below
# are applications submitted by legal entities as opposed to natural persons.
...
def create_if_co_applicant(df):
    df['if_co-applicant'] = np.where(df['co-applicant_race-1'] == 8.0, 0, 1)
    return df
...
def replace_exempt(df):
    collection = ['debt_to_income_ratio', 'property_value', 
                  #'intro_rate_period', 
                  'loan_term', 
                ...
                #'lender_credits',
                 #'discount_points', 
                  #'origination_charges', 
                  #'total_points_and_fees', 'total_loan_costs',
                 #'rate_spread',
                 #'interest_rate', 
                  'loan_to_value_ratio']
...
def replace_numeric_3_conditions_with_exempt(df):
    feature_collection = ['balloon_payment', 'interest_only_payment',
...
def replace_numeric_2_conditions(df):
    feature_collection = ['lien_status', 'preapproval'
                         ]
    substitute_collection = [
...
def fix_column_types(df):
    ...
    df["total_units"] = df["total_units"].replace(["25-49"], 37)
    df["total_units"] = df["total_units"].replace(["5-24"], 15)
    df["total_units"] = df["total_units"].replace(["100-149"], 125)    
...
def impute_na(df):
    df["loan_to_value_ratio"] = df["loan_to_value_ratio"].fillna(df["loan_to_value_ratio"].mean())
...
def derived_sex_to_numeric(df):
    df["derived_sex"] = df["derived_sex"].replace(["Sex Not Available"], 8888)
    df["derived_sex"] = df["derived_sex"].replace(["Male"], 0) 
    df["derived_sex"] = df["derived_sex"].replace(["Female"], 1) 
    df["derived_sex"] = df["derived_sex"].replace(["Joint"], 10)      
    return df

#def conforming_loan_limit_to_numeric(df):
#    conditions = [(df['conforming_loan_limit'] == "NC"), (df['conforming_loan_limit'] == "C"),
#    (df['conforming_loan_limit'] == pd.NA)]
#    choices = [0, 1, 8888]
#    df['conforming_loan_limit'] = np.select(conditions, choices, default=8888)      
#    return df 
...
if __name__ == "__main__":
    ...
    df_preprocessed = derived_sex_to_numeric(
            impute_na(
                fix_column_types(
                    move_target_column_to_the_front(
                        replace_exempt(
    ...
    #df_full_feature_set = df_preprocessed.drop(columns=['if_co-applicant', 'derived_race_revisited', 'derived_age_below_25', 'derived_age_above_62'])

    df_minimal_set = df_preprocessed.drop(columns=['derived_ethnicity', 'derived_race', 'applicant_ethnicity-1', 'co-applicant_ethnicity-1',
    ...
    df_numeric_features = df_minimal_set[["loan_amount", "loan_to_value_ratio", "loan_term", "property_value",
                                        "total_units", "income", "debt_to_income_ratio"]]
    scaled_features = StandardScaler().fit_transform(df_numeric_features.values)
    df_scaled_features = pd.DataFrame(scaled_features, index=df_numeric_features.index, columns=df_numeric_features.columns)

    df_minimal_set = df_minimal_set.drop(columns=["loan_amount", "loan_to_value_ratio", "loan_term", "property_value",
                                     "total_units", "income", "debt_to_income_ratio"])

    df_minimal_set_scaled = pd.concat([df_minimal_set, df_scaled_features], axis=1)

    train, test = train_test_split(df_minimal_set_scaled, test_size=0.2)
    ...
    train.to_csv((Path(args.train_data_rai) / "train_rai.csv"), index=False)
    path_train = [
    {"file": f"{args.train_data_rai}/train_rai.csv"}
    ]
    train_mltable = mltable.from_delimited_files(path_train, include_path_column=False)
    train_mltable = train_mltable.drop_columns(["Column1"])
    train_mltable.save(path=f"{args.train_data_rai}")

    test.to_csv((Path(args.test_data_rai) / "test_rai.csv"))
    path_test = [
    {"file": f"{args.test_data_rai}/test_rai.csv"}
    ]
    test_mltable = mltable.from_delimited_files(path_test, include_path_column=False)
    test_mltable = test_mltable.drop_columns(["Column1"])
    test_mltable.save(path=f"{args.test_data_rai}")    

    #train.to_parquet((Path(args.train_data) / "train.parquet"))
    #test.to_parquet((Path(args.test_data) / "test.parquet"))

    #train.to_parquet((Path(args.train_data_rai) / "train_rai.parquet"))
    #path_train = [
    #{"file": f"{args.train_data_rai}/train_rai.parquet"}
    #]
    #train_mltable = mltable.from_parquet_files(path_train, include_path_column=False)
    #train_mltable.save(path=f"{args.train_data_rai}")

    #test.to_parquet((Path(args.test_data_rai) / "test_rai.parquet"))
    #path_test = [
    #{"file": f"{args.test_data_rai}/test_rai.parquet"}
    #]
    #test_mltable = mltable.from_parquet_files(path_test, include_path_column=False)
    #test_mltable.save(path=f"{args.test_data_rai}")    

    lines = [
        f"Train output path: {args.train_data}",
        f"Test output path: {args.test_data}",
        f"MLTable train output path: {args.train_data_rai}",
        f"MLTable test output path: {args.test_data_rai}"        
    ]
